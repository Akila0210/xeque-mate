...
export async function GET(req: Request, context: { params: { id: string } } | { params: Promise<{ id: string }> }) {
  ...
  const resolved = typeof rawParams?.then === "function" ? await rawParams : (context.params as { id: string });
  const { id } = resolved ?? {};
...
export async function POST(req: Request, context: any) {
  ...
  if (!UUID_REGEX.test(String(id))) {
    return NextResponse.json({ error: "ID do torneio inválido" }, { status: 400 });
  }
  ...
  if (participanteExistente) {
    return NextResponse.json({ error: "Usuário já é participante" }, { status: 400 });
  }
  ...
  if (totalParticipando >= 5) {
    return NextResponse.json({ error: "Limite de 5 torneios participando atingido" }, { status: 400 });
  }
